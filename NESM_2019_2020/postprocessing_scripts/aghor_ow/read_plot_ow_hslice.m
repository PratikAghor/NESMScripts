...
tskip = 1;
vlevel_skip = 50;
ow_t_filename = strcat('nesm_2019_2020_ow_t_hslice_vlevel_', string(vlevel), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)), '.nc');
% u_t_filename = strcat('nesm_2019_2020_u_t_hslice_vlevel_', string(vlevel), '_nt_',string(uv_indxRange(1)), '_', ...
%     string(uv_indxRange(uv_Nt)),'.nc');
% 
% v_t_filename = strcat('nesm_2019_2020_v_t_hslice_vlevel_', string(vlevel), '_nt_',string(uv_indxRange(1)), '_', ...
%     string(uv_indxRange(uv_Nt)),'.nc');
% 
t_arr = ncread(ow_t_filename, 't_arr');
% vlevel = ncread(ow_t_filename, 'vlevel');
ow_t =squeeze(ncread(ow_t_filename, 'ow_t_hslice'));
...
for nt = 2135:tskip:2140
    ...
    sprintf('ow_t arr indx = %d', ow_t_nt)
    sprintf('fig indx = %d', ow_t_nt + nt0 - 1)
        %%% plotting
        figure1 = figure(nt);
        ...
        hold on;
        ow = squeeze(ow_t(ow_t_nt, :, :))./(f0*f0);
        % u_hslice = squeeze(u_t(ow_t_nt, :, :));
        % v_hslice = squeeze(v_t(ow_t_nt, :, :));
    ...
    %%%
        ...
        figname  = [plots_path, 'ow_plots/hslice/nesm_2019_2020_ow'];
