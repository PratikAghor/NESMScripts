...
for nt = 1:Nt
    ...
    kv_t_hslice(nt, :, :) = vinterp(AKt_rho, zr, vlevel); % get hslice at vlevel
end
...
filename = strcat('nesm_2019_2020_kv_t_hslice_vlevel_', string(vlevel) , '_nt_',string(indxRange(1)), '_', ...
...
kv_varid = netcdf.defVar(ncid, 'kv_t_hslice', 'double', [t_len y_len x_len]);
netcdf.putAtt(ncid, kv_varid, 'description', 'kv hslice at const vlevel (Nt)');
netcdf.putAtt(ncid, kv_varid, 'units', 'm^2/s}');
netcdf.putAtt(ncid, kv_varid, 'array dimensions', size(kv_t_hslice));
%close define mode
netcdf.endDef(ncid);
%%
% put values
netcdf.putVar(ncid, time_varid, t_arr); % start from 0
netcdf.putVar(ncid, kv_varid, kv_t_hslice);
% close netcdf file
netcdf.close(ncid);
disp('Done creating kv_t_hslice at const vlevel!')
%--------------------------------------------------------
%%
