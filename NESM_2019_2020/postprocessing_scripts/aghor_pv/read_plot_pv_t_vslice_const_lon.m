...
t_arr = ncread(pv_t_filename, 't_arr');
lon_idx_arr = ncread(pv_t_filename, 'lon_idx_arr');
lon_arr = ncread(pv_t_filename, 'lon_arr');
...
for nt = 1:Nt
    ...
    for j = 1:length(lon_arr)
        ...
        date = date(1:11);
        title(ax1, strcat('(PV)', date), 'interpreter','latex');
        % ax1.TitleHorizontalAlignment = 'left'; % left makes it come to center
        
        hold on;
        LevelList1 = linspace(min(min(rho_pot_vslice)), 36.49, 15);
        LevelList2 = linspace(36.49, 36.8, 15);
        LevelList3 = linspace(36.8, max(max(rho_pot_vslice)), 15);
