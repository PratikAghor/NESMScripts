...
%---------------------------------------
% read speed_t_hslice
filename = strcat('nesm_2019_2020_5km_speed_t_hslice_vlevel_', string(vlevel_arr(1)) , '_nt_',string(indxRange(1)), '_', ...
...
if(strcmp(box, 'AtlantisII_5km')) % covers only Atlantis II
        filename = strcat('nesm_2019_2020_5km_box_avg_speed_t_hslice_vlevel_', string(vlevel_arr(1)), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');
        % ncid = netcdf.create(filename,'CLOBBER');
elseif(strcmp(box, 'NESM_5km')) % covers all three seamounts
        filename = strcat('nesm_2019_2020_5km_bigbox_avg_speed_t_hslice_vlevel_', string(vlevel_arr(1)), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');
        % ncid = netcdf.create(filename,'CLOBBER');
elseif(strcmp(box, 'domain_5km')) % covers all three seamounts
        filename = strcat('nesm_2019_2020_5km_domain_avg_speed_t_hslice_vlevel_', string(vlevel_arr(1)), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');
end
