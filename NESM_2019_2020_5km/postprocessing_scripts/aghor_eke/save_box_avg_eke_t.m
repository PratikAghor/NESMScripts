...
% annual_mean_uvw_file = strcat('nesm_2019_2020_5km_uvw_annual_mean_3d_nt_',string(annual_mean_indxRange(1)), '_', string(annual_mean_indxRange(mean_Nt)),'.nc');
...
%------------------------------------------------
% read speed_t_hslice
filename = strcat('nesm_2019_2020_5km_eke_t_hslice_vlevel_', string(vlevel_arr(1)) , '_nt_',string(indxRange(1)), '_', ...
...
if(strcmp(box, 'AtlantisII_5km')) % covers only Atlantis II
        filename = strcat('nesm_2019_2020_5km_box_avg_eke_t_hslice_vlevel_', string(vlevel_arr(1)), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');
	% ncid = netcdf.create(filename,'CLOBBER');
elseif(strcmp(box, 'NESM_5km')) % covers all three seamounts
        filename = strcat('nesm_2019_2020_5km_bigbox_avg_eke_t_hslice_vlevel_', string(vlevel_arr(1)), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');
	% ncid = netcdf.create(filename,'CLOBBER');
elseif(strcmp(box, 'domain_5km')) % covers all three seamounts
        filename = strcat('nesm_2019_2020_5km_domain_avg_eke_t_hslice_vlevel_', string(vlevel_arr(1)), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');
end
