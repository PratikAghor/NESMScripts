...
VEBF_3d_file = strcat('nesm_2019_2020_5km_VEBF_3d_zlevs_nt_', string(indxRange(1)), '_', string(indxRange(Nt)), '.nc');
...
%--------------------------------------------------------------------------
    ...
    VEBF_filepath = strcat(filepath, 'vebf/nesm_2019_2020_5km_vebf_z_', box, '_nt_', string(indxRange(1)), '_', string(indxRange(Nt)),  '.asc');
...
for i = 1:length(lat_arr)
    ...
    for j = 1:length(lon_arr)
        ...
        for k = 1:N
            ...
            filepath = strcat('../../../compare/energetics/KmKe_VEBF_vert/vslice/');
            VEBF_filepath = strcat(filepath, 'vebf/nesm_2019_2020_5km_vebf_z_lat_', string(lat_arr(i)), '_lon_', string(lon_arr(j)), '_nt_', string(indxRange(1)), '_', string(indxRange(Nt)),  '.asc');
            save(sprintf(VEBF_filepath), 'VEBF1D', '-ascii');
            %--------------------------------------------------------------
