...
%---------------------------------------
% read speed_t_hslice
filename = strcat('sm3_2019_2020_speed_t_hslice_vlevel_', string(vlevel_arr(1)) , '_nt_',string(indxRange(1)), '_', ...
...
if(strcmp(box, 'AtlantisII_1km')) % covers only Atlantis II
        filename = strcat('sm3_2019_2020_box_avg_speed_t_hslice_vlevel_', string(vlevel_arr(1)), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');ncid = netcdf.create(filename,'CLOBBER');
elseif(strcmp(box, 'NESM_1km')) % covers all three seamounts
        filename = strcat('sm3_2019_2020_bigbox_avg_speed_t_hslice_vlevel_', string(vlevel_arr(1)), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');ncid = netcdf.create(filename,'CLOBBER');
end
