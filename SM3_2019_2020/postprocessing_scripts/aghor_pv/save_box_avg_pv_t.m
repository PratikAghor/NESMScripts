...
%--------------------------------------------------
% read speed_t_hslice
filename = strcat('sm3_2019_2020_epv_t_hslice_vlevel_', string(vlevel) , '_nt_',string(indxRange(1)), '_', ...
...
%%
for nt = 1:Nt
    % sprintf(strcat('loading', [dirHR, 'SM1_2019_2020_avg.%05d.nc'], 'file'), indxRange(1, nt))
    % fname = sprintf([dirHR, 'SM1_2019_2020_avg.%05d.nc'], indxRange(1, nt));
    % hisfile = fname;
...
if(strcmp(box, 'AtlantisII_1km')) % covers only Atlantis II
        filename = strcat('sm3_2019_2020_box_avg_pv_t_hslice_vlevel_', string(vlevel), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');
elseif(strcmp(box, 'NESM_1km')) % covers all three seamounts
        filename = strcat('sm3_2019_2020_bigbox_avg_pv_t_hslice_vlevel_', string(vlevel), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');
elseif(strcmp(box, 'domain_1km')) % covers all three seamounts
        filename = strcat('sm3_2019_2020_domain_avg_pv_t_hslice_vlevel_', string(vlevel), '_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');
end
