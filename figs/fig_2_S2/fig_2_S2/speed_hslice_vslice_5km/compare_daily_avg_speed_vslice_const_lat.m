...
for m = 1:numel(models)
    ...
    latval = models{m}.latval;

    c_t_filename = sprintf('%s/%s_avg_speed_t_vslice_const_lat_%.2f_nt_%d_%d.nc', ...
        speed_path, filename_prefix, latval, indxRange(1), indxRange(Nt));
    disp(c_t_filename)
    avg_speed_vslice =squeeze(ncread(c_t_filename, 'avg_speed_t_const_lat'));
    avg_ke_vslice =squeeze(ncread(c_t_filename, 'avg_ke_t_const_lat'));
...
for m = 1:numel(models)
    ...
    X = repmat(lon_rho_vec, size(Z,1), 1);
    Y = Z;  % depth positive down

    pcolor(ax, X, Y, avg_ke_vslice); 
    shading interp;
    ...
    % hline
    plot([lon_rho_vec(1) lon_rho_vec(end)], [hline_depth hline_depth], 'w--', 'LineWidth', 1.5);
    plot([lon_rho_vec(1) lon_rho_vec(end)], [hline_depth_2 hline_depth_2], 'w--', 'LineWidth', 1.5);

    % bathy
    plot(ax, lon_rho_vec, squeeze(Z(1, :)), 'k-', 'LineWidth', 3);
    axis tight;
    ylim(ax, [-5500 0]);

    figname = sprintf('%s/%s_avg_ke_vslice_lat_%.2f_nt_%d_%d.png', ...
        outdir, prefix, models{m}.latval, indxRange(1), indxRange(end));
