...
type = 'r';
coef = 1;
nesm_1km_gridfile = '../fig_3/NESM_grd.nc'; 
nesm_5km_gridfile = '../fig_3/NESM_grd_5km.nc'; 
sm3_1km_gridfile = '../fig_3/SM3_grd.nc'; 

% s_rho = ncread(gridfile, 's_rho');
theta_s=3; % ncread(gridfile, 'theta_s');
theta_b=5; % ncread(gridfile, 'theta_b');
% maskr=pagetranspose(ncread(nesm_gridfile, 'mask_rho'));
% masku=pagetranspose(ncread(nesm_gridfile, 'mask_u'));
% maskv=pagetranspose(ncread(nesm_gridfile, 'mask_v'));

Vtransform=2; % ncread(gridfile, 'Vtransform');
NumLayers = 100;
N = NumLayers;
lat_rho_nesm_1km = pagetranspose(ncread(nesm_1km_gridfile, 'lat_rho'));
lon_rho_nesm_1km = pagetranspose(ncread(nesm_1km_gridfile, 'lon_rho'));
lat_rho_nesm_5km = pagetranspose(ncread(nesm_5km_gridfile, 'lat_rho'));
lon_rho_nesm_5km = pagetranspose(ncread(nesm_5km_gridfile, 'lon_rho'));

depth_nesm_1km = pagetranspose(ncread(nesm_1km_gridfile, 'h'));
depth_nesm_5km = pagetranspose(ncread(nesm_5km_gridfile, 'h'));
depth_sm3_1km = pagetranspose(ncread(sm3_1km_gridfile, 'h'));

hc = 30; % ncread(gridfile, 'hc');

lon_rho_vec_nesm_1km = squeeze(lon_rho_nesm_1km(1, :));
lat_rho_vec_nesm_1km = squeeze(lat_rho_nesm_1km(:, 1));
lon_rho_vec_nesm_5km = squeeze(lon_rho_nesm_5km(1, :));
lat_rho_vec_nesm_5km = squeeze(lat_rho_nesm_5km(:, 1));

zr_const_lat_1km = load('../fig_3/zr_const_lat_38_5_1km.asc');
zr_const_lat_5km = load('../fig_3/zr_const_lat_38_5_5km.asc');
